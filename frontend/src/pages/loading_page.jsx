...
/**
 * LoadingPage / loading_page component - Floating animated splash loading page for eLMS
 */
const LoadingPage = ({ onFinish }) => {
  ...
  useEffect(() => {
    // Smooth progress increment - finishes in 1.2s
  ...
  useEffect(() => {
    ...
    if (progress >= 100) {
      const timeout = setTimeout(() => {
        ...
        }, 200) // matches fade-out animation duration
      }, 100)
  ...
  const content = (
    <div
      className={`fixed inset-0 z-[999999] flex items-center justify-center bg-slate-950/85 backdrop-blur-xl transition-all duration-300 ease-in-out ${isClosing ? 'opacity-0 scale-105 pointer-events-none' : 'opacity-100 scale-100'
        }`}
