  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-[1.65fr_1fr]">
      <section className="surface overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 p-6"><div><p className="eyebrow">Continue learning</p><h2 className="mt-2 text-2xl font-black text-slate-950">Your courses</h2></div><Link to="/my-learning" className="text-sm font-bold text-indigo-600">View all</Link></div>
        {loading ? <p className="p-8 text-sm text-slate-500">Loading your courses…</p> : enrollments.length ? <div className="divide-y divide-slate-100">{enrollments.slice(0, 4).map(item => <div key={item.id} className="flex items-center gap-4 p-5"><img src={departmentCover(item.department_code || 'CSE')} alt="" className="h-16 w-24 rounded-xl object-cover" /><div className="min-w-0 flex-1"><div className="mb-2 flex items-center justify-between gap-3"><h3 className="truncate font-bold text-slate-900">{item.course_title}</h3><span className="text-xs font-bold text-indigo-600">{item.progress || 0}%</span></div><Progress value={item.progress} /></div><Link to={`/learn/${item.course_slug}`} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-slate-950 text-white"><Play size={16} /></Link></div>)}</div> : <div className="p-10 text-center"><BookOpen className="mx-auto text-slate-300" size={38}/><p className="mt-3 text-slate-500">Your learning list is empty.</p><Link to="/departments" className="btn btn-primary mt-5">Find a course</Link></div>}
      </section>
      <section className="surface p-6"><p className="eyebrow">Next up</p><h2 className="mt-2 text-2xl font-black text-slate-950">Study plan</h2><div className="mt-5 space-y-3">{studentTasks.map((task, index) => <div key={task.title} className="rounded-2xl bg-slate-50 p-4"><div className="flex gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white text-xs font-black text-indigo-600 shadow-sm">{index + 1}</span><div><p className="text-sm font-bold text-slate-800">{task.title}</p><p className="mt-1 text-xs text-slate-500">{task.course} · {task.status}</p></div></div></div>)}</div></section>
    </div>
  </>;
}
...
export default function DashboardPage() {
  ...
  return <main className="min-h-[calc(100vh-3.5rem)] bg-[#f6f7fb] py-8 sm:py-12"><div className="page-shell space-y-6">
    <section className="relative overflow-hidden rounded-[2rem] bg-slate-950 text-white shadow-xl"><img src={departmentCover(departmentCode)} alt="" className="absolute inset-0 h-full w-full object-cover opacity-35"/><div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/90 to-slate-950/30"/><div className="relative flex min-h-[280px] items-end justify-between gap-6 p-7 sm:p-11"><div><p className="eyebrow !text-indigo-300">{eyebrow}</p><h1 className="mt-3 max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">Welcome back, {name}.</h1><p className="mt-4 text-lg text-slate-300">{title}</p></div><div className="hidden rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur sm:block"><Bell className="text-indigo-300"/><p className="mt-3 text-xs font-bold uppercase tracking-widest text-slate-300">Role</p><p className="mt-1 font-black capitalize">{role}</p></div></div></section>
